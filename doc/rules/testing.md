...
- `node --test` with no args loads **every** `.js` file under `test/` (including fixtures) and runs it. The default pattern is `**/test/**/*.?(c|m)js`, so this is about the _directory_, not the filename — renaming a helper or moving it to `test/helpers/` does not exempt it.
