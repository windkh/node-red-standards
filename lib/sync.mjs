...
// ESLint 10 needs Node ^20.19.0 || ^22.13.0 || >=24, which is stricter than the >=20.0.0 this
// standard requires of a package at runtime. That only constrains contributors, not consumers,
// so engines stays at >=20.0.0; a contributor on 20.0-20.18 gets an EBADENGINE warning.
// eslint and @eslint/js must move together -- @eslint/js@10 peers on eslint ^10, and mixing
// eslint@10 with @eslint/js@9 silently keeps the v9 recommended set.
const REQUIRED_DEV = {
    '@eslint/js': '^10.0.0',
    globals: '^17.0.0',
...
// Leading major of a semver range ("^9.1.0" -> 9, "~15.2" -> 15, "10.0.1" -> 10).
// Returns null for anything not shaped like a plain range -- git/file/npm-alias specifiers,
// tags, and complex unions are left alone rather than guessed at.
function majorFloor(range) {
    const m = /^\s*[\^~]?\s*(\d+)\./.exec(String(range || ''));
...
const REQUIRED_SCRIPTS = {
    ...
    // Flags matter: --test-force-exit stops lingering keep-alive/undici handles
    // from hanging the run; --test-timeout bounds a stuck test; --test-concurrency=1
    // avoids helper-server contention. No path args — `node --test <dir>` works on
    // Node 20 but is treated as a module specifier on Node 22 ("Cannot find module").
    // Default discovery finds test/**/*.test.js on both and skips test/fixtures.
...
export function runSync(repo, { write = false, force = false, removeLegacy = false } = {}) {
    ...
    const docs = [
        ...
        [
            'doc/architecture/structural-design.md',
            '# Structural design\n\nTODO: modules in lib/ and nodes/ and their responsibilities.\n',
        ],
        [
            'doc/architecture/behavioural-design.md',
            '# Behavioural design\n\nTODO: runtime flows and message handling.\n',
        ],
        [
            'doc/architecture/adr/README.md',
    ...
    if (exists(pkgPath)) {
        ...
        p.scripts = p.scripts || {};
        let changed = false;
        const RUNNER = new Set(['test', 'coverage', 'coverage:check']);
        for (const [k, v] of Object.entries(REQUIRED_SCRIPTS)) {
            if (!p.scripts[k]) {
                p.scripts[k] = v;
                changed = true;
                actions.push({ file: `package.json scripts.${k}`, action: 'add' });
            } else if (RUNNER.has(k) && /\bmocha\b/.test(p.scripts[k])) {
                // migrate legacy mocha runner; leave any existing `node --test ...` (respect customizations)
                p.scripts[k] = v;
                changed = true;
                actions.push({ file: `package.json scripts.${k}`, action: 'migrate mocha -> node:test' });
            }
        }
        p.engines = p.engines || {};
        ...
        for (const [k, v] of Object.entries(REQUIRED_DEV)) {
            if ((p.dependencies || {})[k]) continue;
            const cur = p.devDependencies[k];
            if (!cur) {
                p.devDependencies[k] = v;
                changed = true;
                actions.push({ file: `package.json devDependencies.${k}`, action: 'add' });
            } else if (majorFloor(cur) !== null && majorFloor(cur) < majorFloor(v)) {
                // Raise a range whose floor is below the standard's baseline. Without this the
                // standard can state a minimum it never enforces: `add`-only meant existing repos
                // kept whatever major they had, so a baseline bump reached new adopters only.
                // Lowering is never done -- a repo ahead of the standard stays ahead.
                p.devDependencies[k] = v;
                changed = true;
                actions.push({ file: `package.json devDependencies.${k}`, action: `raise ${cur} -> ${v}` });
            }
        }
    ...
    if (removeLegacy) {
        ...
        if (exists(pkgPath2)) {
            ...
            if (p2.devDependencies && p2.devDependencies['eslint-plugin-prettier']) {
                ...
                actions.push({
                    file: 'package.json devDependencies.eslint-plugin-prettier',
